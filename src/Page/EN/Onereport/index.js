...
function Index() {
  ...
  const [yearFilter, setYearFilter] = useState(currentYear.toString()); // เริ่มต้นด้วยปีปัจจุบัน
            ...
            <div className="data-list">
              {filteredData.map((item, index) => {
                ...
                const pdfUrl = item.file_path.startsWith('http')
                  ? item.file_path
                  : process.env.REACT_APP_PDF_KEY+`/uploads/pdf_files/${item.file_path_en}`;
              ...
              {filteredData.length === 0 && (
                <div className="text-center py-3">
                  <p>No data was found that matches your search.</p>
                </div>
              )}
            </div>
          )}
